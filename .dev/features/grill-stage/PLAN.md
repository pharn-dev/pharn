...
- **`/pharn-grill` emits a `GRILL.md`** in **root `features/<name>/`** (product artifact, mirroring `/pharn-dev-grill`'s `.dev/features/<name>/GRILL.md`). It is written **only when the chain check is GREEN** (the floor gate is a precondition for the advisory interrogation, exactly as `/pharn-plan` produces a PLAN only past its Approved-gate). On a RED chain `/pharn-grill` **HALTs** and writes no `GRILL.md` — it tells the user to re-plan / re-approve.
